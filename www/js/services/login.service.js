    ...
    function requestOauthTokenSuccessCallback(response) {
        ...
        promise = $q(function(resolve, reject) {
            if(!response.data.exception) {
                ...
                login.expiresIn = response.data.expires_in;
                login.tokenType = response.data.token_type;
                // in this case the 'scope' must be 'profile.basicprofile.me,profile.accountprofile.me'
                // in order to be authorized to create citizens' POIs in the corresponding dataset (in the Open Data Stack)
                login.scope = response.data.scope;
